...
pub struct ZoneCache<'a, const N: usize> {
    /// Zone과 Zone에 속한 Cell 목록을 Vec로 정렬. bool은 최근 수정 여부
    zone: IndexKeyMap<Zone, Vec<(&'a Cell<N>, RelaxedBool)>>,
...
impl<'a, const N: usize> ZoneCache<'a, N> {
    ...
    #[must_use]
    fn get_zone_map(t: &'a TableLock<N>) -> IndexKeyMap<Zone, Vec<(&'a Cell<N>, RelaxedBool)>> {
        let mut index_vec: IndexKeyMap<Zone, Vec<(&'a Cell<N>, RelaxedBool)>> =
            IndexKeyMap::with_capacity(N * N);
        ...
        for cell in t {
            for z in &cell.zone_vec {
                ...
                row.push((
                    cell,
                    RelaxedBool::new(read.read_from_cell(cell).true_cnt() != N),
                ));
            }
        }
        ...
        index_vec
            .iter_mut()
            .for_each(|(_, v)| v.par_sort_unstable_by_key(|&(c, _)| c));
        index_vec
    }

    #[must_use]
    fn get_connected_zone(
        ordered_zone: &IndexKeyMap<Zone, Vec<(&'a Cell<N>, RelaxedBool)>>,
        ...
        for (z1, _) in ordered_zone {
            for (z2, c2_list) in ordered_zone {
                let connected = c2_list.iter().any(|(c2, _)| c2.zone_set.contains(z1));
    ...
    #[must_use]
    #[inline]
    pub fn zone(&self) -> &IndexKeyMap<Zone, Vec<(&'a Cell<N>, RelaxedBool)>> {
        &self.zone
    }
